...
public class GameSceneScoreManager : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			//前回のスコアと現在のスコアを比較し、現在のスコアと差異があった場合　もしくは2フレーム目の場合
			if (ScoreManager.GetOldScore() != ScoreManager.GetNowScore() || nNotNULLLoadCount < 3)
			{
				//スコア表示用プレートにスコア値を適応する
				ScorePlateControlNumber();

				//前回のスコアと現在のスコアを同期させる
				ScoreManager.SetEqualScore();
			}
		}
	}
	...
	void ScorePlateControlNumber()
	{
		...
		for (int nDigitLoop = 0; nDigitLoop < ConstantScore.RANKING_DIGIT_MAX; nDigitLoop++)
		{
			...
			//値を使用するPlate名を指定する
			sBuffer = "ScoreNumber" + (nDigitLoop + 1);
			...
			for (int nLoopS = 0; nLoopS < ScorePlates.Count; nLoopS++)
			{
				...
				if (ScorePlates[nLoopS].name.Equals(sBuffer))
				{
					...
					if (nBuffer - 1 < 0)
					{
						...
						//Debug.Log("スコアループ：" + nDigitLoop + "　適応桁数：" + nDigitLoop + "　送る値：固定9");
					}
					//0以上なら通常適応
					else
					{
						...
						//Debug.Log("スコアループ：" + nDigitLoop + "　適応桁数：" + nDigitLoop + "　送る値" + (nBuffer - 1));
					}
